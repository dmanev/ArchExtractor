...
// 
//    Rev 1.30   Feb 14 2011 17:58:12   amanevd
// Cm026229: Return rev. 1.27
// 
//    Rev 1.29   Jun 21 2010 11:09:52   amanevd
// Cm009203: EEP_CDVerlog is not increased on SC GND
// 
//    Rev 1.28   Jun 16 2010 15:22:14   adzhelp
// Cm008639: Merge rev. 1.27 and rev. 1.26
// 
...
static MEM_TYPE void DATVlgSerialLineTest(void)
{
    ...
    {
        #ifdef X90_PROJECT // amanevd: Add 42 NOPs to preserve original location
        NOP();NOP();NOP();NOP();NOP();
        NOP();NOP();NOP();NOP();NOP();
        NOP();NOP();NOP();NOP();NOP();
        NOP();NOP();NOP();NOP();NOP();
    ...
    {
        //CCOV: amanevd: Cannot be covered because this function is left in order to preserve original location
        // function to be removed with TCR: 
        #ifdef X90_PROJECT // amanevd: Add 17 NOPs to preserve original location
        NOP();NOP();NOP();NOP();NOP();
        NOP();NOP();NOP();NOP();NOP();
        NOP();NOP();NOP();NOP();NOP();
        NOP();NOP();
        ...
        if(cFalse != bTempL)
        {
            #ifdef X90_PROJECT // amanevd: Add 3 NOPs to preserve original location
            NOP();NOP();NOP();
            #endif
        }
    }
...
MEM_TYPE void DATVlgControl(const tCtrl Ctrl)
{
    ...
    switch (Ctrl)
    {
        ...
        #ifdef X90_PROJECT // amanevd: Add 2 NOPs to preserve original location
        NOP();NOP();
        #endif
        tGraphState = TOSSeqGraphState(cTOSSeqGraphIdDATVlgOutputProtection);
        if (cTOSSeqActiveGraph != tGraphState)
        {
            #ifdef X90_PROJECT // amanevd: Add 27 NOPs to preserve original location
            NOP();NOP();NOP();NOP();NOP();
            NOP();NOP();NOP();NOP();NOP();
            NOP();NOP();NOP();NOP();NOP();
            NOP();NOP();NOP();NOP();NOP();
...
{
    const BOOL bFrameEmissionStopped = LIBVlg2FrameEmissionStopped();

    return (    (cFalse != bFrameEmissionStopped)
...
{   
    #ifdef X90_PROJECT // amanevd: Add 1 NOPs to preserve original location 
    NOP();
    #endif
    if(u8LibVlg2TickCounter > 0)
    {
        #ifdef X90_PROJECT // amanevd: Add 5 NOPs to preserve original location 
        NOP();NOP();NOP();NOP();NOP();
        #endif
        LIBVlg2TimerTick();
